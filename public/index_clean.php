...
if (FileHelper::exists($maintenanceFile)) {
    ...
    if ($maintenanceData['enabled'] ?? false) {
        ...
        include __DIR__ . '/../views/errors/503.php';
        exit;
    }
}
...
set_exception_handler(function($exception) {
    ...
    if (app_debug()) {
        ...
        http_response_code(500);
        include template_path('errors/500');
    }
});
...
if (app_debug()) {
    ...
    register_shutdown_function(function() use ($startTime, $startMemory) {
        ...
        if (app_debug()) {
            echo '<!-- Debug: Execution time: ' . $executionTime . 'ms -->';
            echo '<!-- Debug: Memory used: ' . round($memoryUsed / 1024 / 1024, 2) . 'MB -->';
        }
    });
}
